import { TMove, TMoveResult, TMoveReturn, TNotation } from "check-board";
...
class BoardRuntime {
  ...
  async move(args: TMove): Promise<TMoveReturn> {
    ...
    try {
      ...
      if (move?.isKingsideCastle()) {
        ...
        const rank = turn
          ...
          : parseInt(to.charAt(1)) - 1;
        const enPassant = `${to.charAt(0)}${rank}` as TNotation;
