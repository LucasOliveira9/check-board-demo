...
import { hoverTest, runAllClearStressTests } from "../tests/test";
import { Chess } from "chess.js";

type TMyEventsContext = TGetterBoardEventContext & {
  chess?: Chess;
  client?: React.RefObject<Client | null>;
};

const events: TBoardEvents<TMyEventsContext> = {
  onPointerSelect: ({
    y,
    x,
    squareSize,
    ...
    if (!square || !chess || !client || !getPieces) return;
    const poss = chess.moves({
      square: square.notation,
      verbose: true,
    });

    const possN: { to: TNotation; enpassant?: TNotation }[] = poss.map((x) => {
      if (x.isEnPassant())
        return {
          to: x.to as TNotation,
          enpassant: `${x.to[0]}${x.from[1]}` as TNotation,
        };
      return { to: x.to as TNotation };
    });

    const drawPoss = (ctx: TSafeCtx) => {
    ...
    const drawOnDanger = (ctx: TSafeCtx) => {
      //test
    ...
    const drawSelect = (ctx: TSafeCtx) => {
      // Detecta se tocará a borda do canvas
      const isAtBorder =
        x <= 0 || y <= 0 || x + squareSize >= size || y + squareSize >= size;
    ...
    const drawTest = (ctx: TSafeCtx) => {
      ctx.fillStyle = "green";
      ctx.font = "20px monospace";
      ctx.fillText("TEST", 350, 90);
    };

    const drawOtherTest = (ctx: TSafeCtx) => {
      draw({
        layer: "overlay",
        onDraw: (ctx) =>
          runAllClearStressTests(
            ctx,
            700,
            700,
            imagens?.bB instanceof HTMLImageElement
              ? (Array.from(Object.values(imagens)) as HTMLImageElement[])
              : [],
          ),
      });
    };
