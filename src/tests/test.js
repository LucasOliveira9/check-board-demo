...
/* -------------------------
   ...
   ------------------------- */
function drawPath2DStress(ctx, width, height, seed = 12345, intensity = 1) {
    ...
    for (let i = 0; i < loops; i++) {
        ...
        if (rng() > 0.5) {
            p.moveTo(cx, cy);
            p.bezierCurveTo(cx + r * 0.3, cy - r, cx - r * 0.7, cy + r * 0.7, cx + r, cy + r * 0.2);
        }
        else {
            p.moveTo(cx, cy);
        ...
        ctx.strokeStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.2, 1)})`;
        ctx.fillStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.05, 0.9)})`;
...
/* -------------------------
   ...
   ------------------------- */
function drawImageStress(ctx, width, height, images, seed = 777, intensity = 1) {
    const rng = makeRng(seed);
    ...
    for (let i = 0; i < Math.floor(60 * intensity); i++) {
        ...
        const sw = Math.floor(randBetween(rng, Math.max(8, img.width * 0.05), img.width - sx));
        const sh = Math.floor(randBetween(rng, Math.max(8, img.height * 0.05), img.height - sy));
    ...
    for (let i = 0; i < Math.floor(400 * intensity); i++) {
        ...
        try {
            ctx.drawImage(img, 0, 0, Math.min(img.width, s), Math.min(img.height, s), dx, dy, s, s);
        }
        catch {
            console.warn("s");
        }
    ...
    for (let i = 0; i < Math.floor(80 * intensity); i++) {
        ...
        ctx.fillStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.06, 0.4)})`;
...
/* -------------------------
   ...
   ------------------------- */
function drawTextStress(ctx, width, height, seed = 999, intensity = 1) {
    const rng = makeRng(seed);
    ...
    for (let i = 0; i < Math.floor(2000 * intensity); i++) {
        ...
        ctx.font = `${fontSize}px ${rng() > 0.5 ? "monospace" : rng() > 0.5 ? "serif" : "sans-serif"}`;
        ctx.fillStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.2, 1)})`;
        const text = phrases[Math.floor(rng() * phrases.length)];
        ctx.fillText(text, randBetween(rng, -20, width + 20), randBetween(rng, -20, height + 20));
        if (rng() > 0.7) {
            ctx.strokeStyle = ctx.fillStyle;
            ctx.lineWidth = Math.max(0.5, randBetween(rng, 0.3, 3));
            ctx.strokeText(text, randBetween(rng, -20, width + 20), randBetween(rng, -20, height + 20));
        }
    }
}
/* -------------------------
   ...
   ------------------------- */
function drawShapesStress(ctx, width, height, seed = 555, intensity = 1) {
    const rng = makeRng(seed);
    // large shapes
    for (let i = 0; i < Math.floor(800 * intensity); i++) {
        ctx.fillStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.05, 0.95)})`;
        ctx.strokeStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.1, 1)})`;
        ...
        if (rng() > 0.6) {
            ctx.beginPath();
            ctx.arc(x + w / 2, y + h / 2, Math.max(1, randBetween(rng, 3, Math.min(width, height) * 0.25)), 0, Math.PI * 2);
            if (rng() > 0.5)
                ctx.fill();
            else
                ctx.stroke();
    ...
    for (let i = 0; i < Math.floor(20000 * intensity); i++) {
        ctx.fillStyle = `rgba(${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${Math.floor(randBetween(rng, 0, 255))},${randBetween(rng, 0.25, 1)})`;
...
/* -------------------------
   ...
   ------------------------- */
function drawPixelBlockStress(ctx, width, height, seed = 4242, intensity = 1) {
    const rng = makeRng(seed);
...
/* -------------------------
   ...
   ------------------------- */
function drawPatternStress(ctx, width, height, images, seed = 1337, intensity = 1) {
    const rng = makeRng(seed);
    if (!images || images.length === 0)
        return;
    for (let i = 0; i < Math.floor(120 * intensity); i++) {
        ...
        try {
            ...
            if (!p)
                continue;
            ctx.fillStyle = p; // TS typing; runtime accepts CanvasPattern
...
/* -------------------------
   ...
   ------------------------- */
async function runAllClearStressTests(ctx, width, height, images = [], opts) {
    ...
    drawShapesStress(ctx, width, height, seed ^ 0x7777, Math.max(0.5, intensity * 0.6));
}
const testFen = [
